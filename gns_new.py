...
# Number of classes (including background)
    NUM_CLASSES = 1 + 5  # Background + jagung+gulma
...
class gnsDataset(utils.Dataset):
    ...
    def load_gns(self, dataset_dir, subset):
        ...
        # Add classes. We have two classes to add.
        self.add_class("gns",1,"Ageratum sp.")
        self.add_class("gns",2,"Commelina sp.")
        self.add_class("gns",3,"Eleusine sp.")
        self.add_class("gns",4,"Sacciolepis sp.")
        self.add_class("gns",5,"Jagung(Zea Mays)")
            
        self.class_name_to_ids={'Ageratum sp.':1,'Commelina sp.':2,'Eleusine sp.':3, 'Sacciolepis sp.':4,'Jagung(Zea Mays)':5}
        ...
        for a in annotations:
            ...
            if type(a['regions']) is dict:
                ...
                class_names = [list(r['region_attributes']['nama'].keys())[0] for r in a['regions'].values()]
            else:
                polygons = [r['shape_attributes'] for r in a['regions']] 
                class_names = [list(r['region_attributes']['nama'].keys())[0] for r in a['regions']]
...
def train(model):
    ...
    augment = iaa.SomeOf((0, 2), [
        iaa.Fliplr(0.5),
        iaa.Flipud(0.5),
        iaa.OneOf([iaa.Affine(rotate=90),
                   iaa.Affine(rotate=180),
                   iaa.Affine(rotate=270)]),
        iaa.Affine(scale={"x": (1, 1.2), "y": (0.7, 1)}),
        iaa.Multiply((0.8, 1.5)),
        iaa.GaussianBlur(sigma=(0.0, 2.0))
    ])
    model.train(dataset_train, dataset_val,
                learning_rate=config.LEARNING_RATE,
                epochs=200,augmentation=augment,
                layers='heads')
